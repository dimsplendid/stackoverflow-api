...
use persistance::{
    ...
    questions_dao::{QuestionsDaoImpl, self},
};
...
async fn rocket() -> _ {
    ...
    rocket::build()
        ...
        .manage(Box::new(questions_dao)) // pass in `questions_dao` as a boxed trait object. hint: you must cast `questions_dao` to a trait object.
